...
import type { Tool, ShapeMode, Point, DrawAction, Shape3DType } from "@/lib/drawingTypes";
...
export function DrawingCanvas({
  ...
  // Draw a 3D shape
  const drawShape3D = useCallback(async (
    ...
    try {
      ...
      // Draw the rendered 3D shape onto the canvas
  ...
  const handleMouseDown = useCallback(async (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (tool === 'shape3d') {
      ...
      // Render and place the 3D shape
      const size = Math.max(brushSize * 10, 100); // Minimum 100px size
      const defaultRotation = { x: 0.5, y: 0.5, z: 0 }; // Default rotation for better visibility
      await drawShape3D(ctx, point.x, point.y, size, defaultRotation);
      ...
      onDrawAction({
        ...
        endPoint: point,
        shape3dType,
        shape3dRotation: defaultRotation,
      });
      return;
    }
